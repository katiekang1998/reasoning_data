...
def make_supervised_data_module(output_dir, train_type, learning_rate, batch_size, epochs, tokenizer: transformers.PreTrainedTokenizer) -> Dict:
    ...
    train_levels_new = train_levels_orig[orig_data_easy_idxs]
    ...
    if "0copies" in train_type:
        num_copies = 0
    elif "1copies" in train_type and "11copies" not in train_type:
        num_copies = 1
    elif "3copies" in train_type and "13copies" not in train_type:
        num_copies = 3
    elif "7copies" in train_type:
        num_copies = 7
    elif "11copies" in train_type:
        num_copies = 11
    elif "13copies" in train_type:
        num_copies = 13
    else:
        raise Exception("Invalid train type")


    if "25percentile" in train_type:
        percentile = 25
    else:
        raise Exception("Invalid train type")
    ...
    if dist.get_rank() == 0:
        ...
        # threshold = np.percentile(ifd_scores, percentile)
        # orig_data_subsample_idxs = np.where(ifd_scores>=threshold)[0]
        orig_data_subsample_idxs = np.argsort(train_levels_new)[int(percentile/100*len(train_levels_new)):]
        amrith_data_subsample_idxs = np.where([elem in orig_data_subsample_idxs for elem in amrith_data_orig_idxs])[0]
        amrith_data_subsample_idxs = np.random.choice(amrith_data_subsample_idxs, int(len(train_questions)*num_copies), replace=False)
...
def train():
    ...
    project_name = "math_amrith_easy_deepseek_huer"
