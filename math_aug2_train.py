...
def make_supervised_data_module(output_dir, easy_ratio, hard_ratio, num_train_points, tokenizer: transformers.PreTrainedTokenizer) -> Dict:
    ...
    subsample_idxs = np.concatenate([subsample_easy_idxs, subsample_hard_idxs])
...
def train():
    ...
    if args.save_intermediate:
        save_steps = num_train_points//10
        save_strategy = "steps"
    else:
        save_steps = None
        save_strategy = "no"
    ...
    assert(hard_ratio+easy_ratio == 1)
    
    project_name = "math_aug2"
    if num_epochs ==10:
        run_name  =f"easy{easy_ratio:.2f}_hard{hard_ratio:.2f}_total{num_train_points}"
    else:
        run_name  =f"easy{easy_ratio:.2f}_hard{hard_ratio:.2f}_total{num_train_points}_epochs{num_epochs}"
    ...
    data_module = make_supervised_data_module(output_dir, easy_ratio, hard_ratio, num_train_points, tokenizer=tokenizer)
